...
class ParallelVoiceAssistant:
    ...
    def __init__(
        ...
        self.recorder = _recorder if _recorder is not None else StreamingRecorder(
        ...
        if whisper_server:
            ...
            self.stt = ParallelSTT(
                ...
                sample_rate=sample_rate,
                emit_partials=True,
                vad_aggressiveness=0,  # NOTE: raise to 1-3 in noisy environments
        ...
        self.tts = BufferedTTS(
            ...
            playback_cmd=["aplay", "-t", "raw", "-f", "S16_LE", "-r", "22050", "-c", "1", "-"],
        ...
        self._silent_chunks_before_stop = max(
            2, int(math.ceil(self._silence_timeout / max(0.1, self._chunk_duration)))
        )

        self._noise_blacklist = {
            "wind blowing", "bird chirping", "blank_audio", "blank audio",
            "[blank_audio]", "(wind blowing)", "(bird chirping)",
        }
        ...
        self._awaiting_transcript_chunk_limit = max(
            2, int(math.ceil(1.0 / max(0.1, self._chunk_duration)))
        )
    ...
    def _queue_intermediate_transcription(self, reason: str) -> None:
        # NOTE: streaming backends (Vosk) finalize continuously — forcing a mid-utterance flush causes duplicates
    ...
    def _is_speech_chunk(self, audio_chunk: np.ndarray) -> bool:
        ...
        # NOTE: hysteresis band avoids calling the (expensive) VAD on borderline chunks
    ...
    def _stt_pipeline(self) -> None:
        ...
        try:
            while self.recorder.recording or not self.recorder.chunk_queue.empty():
                ...
                self._cmd_audio.append(audio_chunk.copy())  # keep rolling window for command re-decode
        ...
        while not self.stt_futures.empty():
            ...
            if is_noise or not normalized:
                if had_activity and not normalized:
                    print(f"[STT] Chunk {res_chunk_id}: (speech detected, awaiting transcription)")
    ...
    def _llm_pipeline(self) -> None:
        ...
        while not self._stt_done.is_set() or not self.llm_futures.empty():
            ...
            if self.tts:
                # flush any tokens the streaming callback hasn't spoken yet
                if self._spoken_buf:
            ...
            # fallback path (no Piper): sentence-by-sentence via espeak
            ...
            for sentence in sentences:
                ...
                if (self.stats.recording_stop_to_first_tts_latency is None
    ...
    def _maybe_handle_model_switch(self, text: str) -> bool:
        ...
        if self._switch_require_codeword:
            ...
            # NOTE: require framing verb or codeword to prevent accidental triggers in normal speech
            if not (has_switch_verb or has_codeword):
                return False

        if not (wants_pro or wants_lite):
            # NOTE: open-vocab decode may miss "smart"/"fast" — retry with a restricted grammar
    ...
    def _print_stats(self, elapsed: float) -> None:
        ...
        if self.stats.recording_to_first_llm_latency is not None:
            print(f"Recording -> first LLM response: {self.stats.recording_to_first_llm_latency:.2f}s")
        else:
            print("Recording stop -> first LLM response: n/a")

        if self.stats.recording_stop_to_first_tts_latency is not None:
            print(f"Recording stop -> first TTS audio: {self.stats.recording_stop_to_first_tts_latency:.2f}s")
        else:
